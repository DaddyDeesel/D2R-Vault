...
assert.ok(!styledPost.includes('[color=gold]25 fg each[/color]'));
let compatibility=logic.analyzeJspPost(styledPost);
assert.equal(compatibility.valid,true);
assert.equal(compatibility.colorTags,1);
compatibility=logic.analyzeJspPost('x'.repeat(4001));
assert.equal(compatibility.valid,false);
assert.ok(compatibility.errors.some(error=>error.includes('characters over')));
compatibility=logic.analyzeJspPost('[color=gold]x[/color]'.repeat(21));
assert.equal(compatibility.valid,false);
assert.equal(compatibility.colorTags,21);
assert.ok(compatibility.errors.some(error=>error.includes('20')));
assert.equal(logic.analyzeJspPost('[size=23]large[/size]').valid,true);
assert.equal(logic.analyzeJspPost('[size=24]too large[/size]').valid,false);
assert.equal(logic.analyzeJspPost('[b][i]broken[/b][/i]').valid,false);
const limitedPreview=logic.parseBBCode('[color=gold]x[/color]'.repeat(21));
assert.ok(JSON.stringify(limitedPreview).includes('[color=gold]'));
