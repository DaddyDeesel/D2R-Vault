...
> **Current release: [D2R Treasure Vault v0.3.2](https://github.com/DaddyDeesel/D2R-Vault/releases/tag/v0.3.2)** · Windows 10/11 x64

> [!IMPORTANT]
> **D2R Treasure Vault requires a [D2R-Manager](https://github.com/cartographerwanders/d2rmanager-cartographer-wanderer/) FULL LICENSE to work.** Treasure Vault reads the `items.db` generated by D2R-Manager; it does not capture Diablo II: Resurrected inventory data itself. Install and run D2R-Manager first, then connect Treasure Vault to its `items.db` file. For installation help and community support, join the [D2R-Manager Discord](https://discord.gg/ppgE9WTzf5).

## Download

1. Open the [v0.3.2 release](https://github.com/DaddyDeesel/D2R-Vault/releases/tag/v0.3.2).
2. Under **Assets**, download `D2R-Treasure-Vault-0.3.2-Windows-x64.zip`.
3. Extract the entire ZIP into its own writable folder.
4. Open `D2R-Treasure-Vault.exe`. Keep the `_internal` folder beside it.
5. In **Settings**, select the `items.db` created by [D2R-Manager](https://github.com/cartographerwanders/d2rmanager-cartographer-wanderer/).
...
- Validate nested BBCode, supported colors, and d2jsp text sizes from 0 through 23. The preview exposes color markup beyond the current cap so failures are visible before posting.
...
The post-size and color limits are based on current d2jsp behavior reported in its [post-size discussion](https://forums.d2jsp.org/topic.php?f=34&t=104601266) and [color-cap discussion](https://forums.d2jsp.org/topic.php?f=34&t=109458233). Supported tag syntax and the 0–23 size range follow the [d2jsp BBCode guide](https://forums.d2jsp.org/topic.php?f=156&o=0&t=3668614). d2jsp can change these limits, so the app labels them as compatibility checks rather than forum guarantees.
...
Special thanks and full credit to **Wanderer** for the amazing work on [D2R-Manager](https://github.com/cartographerwanders/d2rmanager-cartographer-wanderer/), including its Diablo II: Resurrected overlay and botting tools. D2R Treasure Vault is possible because D2R-Manager captures and organizes the inventory data that powers it.

Visit the [D2R-Manager Discord](https://discord.gg/ppgE9WTzf5) to connect with its community, get help, and follow the project.
